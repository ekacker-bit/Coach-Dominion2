...
test("030K is cache-busted and production gated", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030[KLMN]\.1/);
  ...
  assert.match(app, /register\("\/sw\.js\?v=030[klmn]"/);
  assert.match(health, /release: "030[KLMN]\.1"/);
  assert.match(health, /weeklyReconciliation: "finalized-evidence-to-committed-week"/);
  assert.match(workflow, /npm run test:030[klmn]/);
  assert.match(workflow, /--expected-release 030[KLMN]\.1/);
});
