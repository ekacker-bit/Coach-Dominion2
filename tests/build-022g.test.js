...
test("022G rotates every mutable shell asset", () => {
  assert.match(html, /styles\.css\?v=(?:022g|(?:023[abcdef]|024[abcdefghijkl]))/);
  assert.match(html, /mobile-command\.js\?v=022g/);
  assert.match(html, /app\.js\?v=(?:022g|(?:023[abcdef]|024[abcdefghijkl]))/);
  assert.match(worker, /coach-dominion-(?:022g|(?:023[abcdef]|024[abcdefghijkl]))-v1/);
  assert.match(worker, /styles\.css\?v=(?:022g|(?:023[abcdef]|024[abcdefghijkl]))/);
  assert.match(worker, /mobile-command\.js\?v=022g/);
  assert.match(worker, /app\.js\?v=(?:022g|(?:023[abcdef]|024[abcdefghijkl]))/);
});
