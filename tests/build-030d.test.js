...
test("030D release identity, shell, and production gate advance together", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030[DEF]\.1"/);
  ...
  assert.match(health, /release: "030[DEF]\.1"/);
  assert.match(workflow, /npm run test:030[def]/);
  assert.match(workflow, /--expected-release 030[DEF]\.1/);
