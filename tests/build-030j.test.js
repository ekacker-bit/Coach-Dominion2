...
test("030J is cache-busted and production gated", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030[JKLMNOPQRSTU]\.1/);
  ...
  assert.match(app, /register\("\/sw\.js\?v=030[jklmnopqrstu]"/);
  assert.match(health, /release: "030[JKLMNOPQRSTU]\.1"/);
  assert.match(health, /decisionProof: "outcome-verified"/);
  assert.match(workflow, /npm run test:030[jklmnopqrstu]/);
  assert.match(workflow, /--expected-release 030[JKLMNOPQRSTU]\.1/);
});
