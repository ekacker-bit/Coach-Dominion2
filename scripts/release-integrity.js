...
check("unified fuel command", html.includes('class="fuel-command-center"') && app.includes("DominionFuelCommand.buildFuelCommand"), "023B unified Fuel surface is missing");
check("fuel command engine", size("assets/js/fuel-command.js") >= 5500 && read("assets/js/fuel-command.js").includes('const VERSION = "023B.1"'), "023B Fuel engine appears incomplete");
...
check("stylesheet version", html.includes('/assets/styles.css?v=023b'), "app.html is not using the 023B stylesheet");
check("application version", html.includes('/assets/js/app.js?v=023b'), "app.html is not using the 023B application");
check("cache version", worker.includes('coach-dominion-023b-v1'), "service-worker cache was not rotated");
check("cached stylesheet", worker.includes('/assets/styles.css?v=023b'), "service worker is caching the wrong stylesheet");
check("cached application", worker.includes('/assets/js/app.js?v=023b'), "service worker is caching the wrong application");
...
check("cached fuel command", worker.includes('/assets/js/fuel-command.js?v=023b'), "service worker is not caching the Fuel engine");
