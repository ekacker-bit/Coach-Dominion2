...
assert.match(html, />Your execution queue</);
assert.match(html, /id="daily-queue-progress"/);
assert.match(html, /class="card today-standards-duty today-attention-card legacy-today-surface"/);
assert.match(html, /class="card today-operating-sequence legacy-today-surface"/);
...
assert.match(js, /function buildCurrentDailyExecutionQueue/);
assert.match(js, /daily-execution-queue:/);
assert.match(js, /action === "complete_recovery"/);
...
assert.match(css, /\.legacy-today-surface\{display:none!important\}/);
assert.match(css, /\.daily-execution-step\.current/);
