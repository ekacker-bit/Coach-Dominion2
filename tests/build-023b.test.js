...
test("023B rotates the mutable shell and caches both Fuel engines", () => {
  assert.match(html, /styles\.css\?v=(?:023[bcdef]|024[abcdefghijklm])/);
  assert.match(html, /fuel-command\.js\?v=(?:023[bcdef]|024[abcdefghijklm])/);
  assert.match(html, /app\.js\?v=(?:023[bcdef]|024[abcdefghijklm])/);
  assert.match(worker, /coach-dominion-(?:023[bcdef]|024[abcdefghijklm])-v1/);
  assert.match(worker, /fuel-calendar\.js\?v=023b/);
  assert.match(worker, /fuel-command\.js\?v=(?:023[bcdef]|024[abcdefghijklm])/);
});
