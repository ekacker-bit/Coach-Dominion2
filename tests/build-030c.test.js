...
test("030C is identifiable, cached, gated, and does not expose release language", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030[CDEFGHIJKLMNOPQR]\.1"/);
  ...
  assert.match(health, /release: "030[CDEFGHIJKLMNOPQR]\.1"/);
  assert.match(workflow, /npm run test:030[cdefghijklmnopqr]/);
  assert.match(workflow, /--expected-release 030[CDEFGHIJKLMNOPQR]\.1/);
