...
test("023C rotates and caches the mutable shell", () => {
  assert.match(html, /styles\.css\?v=(?:023[cdef]|024[abcdef])/);
  assert.match(html, /intermittent-fasting\.js\?v=023[cd]/);
  assert.match(html, /fuel-command\.js\?v=(?:023[cdef]|024[abcdef])/);
  assert.match(html, /app\.js\?v=(?:023[cdef]|024[abcdef])/);
  assert.match(worker, /coach-dominion-(?:023[cdef]|024[abcdef])-v1/);
