...
test("022E loads the verdict engine between plan command and application", () => {
  ...
  const application = html.search(/\/assets\/js\/app\.js\?v=(?:022[e-g]|(?:023[abcdef]|024[abcdefghijkl]))/);
...
test("022E is responsive, versioned, and available offline", () => {
  ...
  assert.match(html, /styles\.css\?v=(?:022[e-g]|(?:023[abcdef]|024[abcdefghijkl]))/);
  assert.match(worker, /coach-dominion-(?:022[e-g]|(?:023[abcdef]|024[abcdefghijkl]))-v1/);
  assert.match(worker, /observation-verdict\.js\?v=022e/);
  assert.match(worker, /app\.js\?v=(?:022[e-g]|(?:023[abcdef]|024[abcdefghijkl]))/);
});
