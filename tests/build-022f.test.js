...
test("022F loads the closeout engine before the ritual and application", () => {
  ...
  const application = html.search(/\/assets\/js\/app\.js\?v=(?:022[fg]|(?:023[abcdef]|024[ab]))/);
...
test("022F is responsive, versioned, and available offline", () => {
  ...
  assert.match(html, /styles\.css\?v=(?:022[fg]|(?:023[abcdef]|024[ab]))/);
  assert.match(worker, /coach-dominion-(?:022[fg]|(?:023[abcdef]|024[ab]))-v1/);
  assert.match(worker, /daily-closeout\.js\?v=022f/);
  assert.match(worker, /daily-ritual\.js\?v=022f/);
  assert.match(worker, /app\.js\?v=(?:022[fg]|(?:023[abcdef]|024[ab]))/);
});
