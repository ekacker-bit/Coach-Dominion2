...
test("030Q is cache-busted and production gated", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030[QRSTUVW]\.1"/);
  assert.match(html, /app\.js\?v=[^"]*-030q/);
  assert.match(worker, /030q-calendar-restore-repair/);
  assert.match(worker, /app\.js\?v=[^"]*-030q/);
  assert.match(app, /register\("\/sw\.js\?v=030[qrstuvw]"/);
  assert.match(health, /release: "030[QRSTUVW]\.1"/);
  assert.match(health, /calendarRestore: "active-week-fail-safe"/);
  assert.match(workflow, /npm run test:030[qrstuvw]/);
  assert.match(workflow, /--expected-release 030[QRSTUVW]\.1/);
