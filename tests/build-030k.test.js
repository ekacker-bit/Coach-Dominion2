...
test("030K is cache-busted and production gated", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030[KLMNOP]\.1/);
  ...
  assert.match(app, /register\("\/sw\.js\?v=030[klmnop]"/);
  assert.match(health, /release: "030[KLMNOP]\.1"/);
  assert.match(health, /weeklyReconciliation: "finalized-evidence-to-committed-week"/);
  assert.match(workflow, /npm run test:030[klmnop]/);
  assert.match(workflow, /--expected-release 030[KLMNOP]\.1/);
});
