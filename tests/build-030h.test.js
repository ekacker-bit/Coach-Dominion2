...
test("030H is cache-busted and release gated", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030[HIJKL]\.1/);
  assert.match(html, /beta-state-integrity\.js\?v=030h/);
  assert.match(worker, /030h-beta-state-integrity/);
  assert.match(app, /register\("\/sw\.js\?v=030[hijkl]"/);
  assert.match(health, /release: "030[HIJKL]\.1"/);
  assert.match(workflow, /npm run test:030[hijkl]/);
  assert.match(workflow, /--expected-release 030[HIJKL]\.1/);
});
