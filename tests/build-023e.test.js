...
test("023E is responsive and rotates all mutable release assets", () => {
  ...
  assert.match(html, /styles\.css\?v=(?:023[ef]|024[abcdefghijkl])/);
  assert.match(html, /meal-execution\.js\?v=(?:023[ef]|024[abcdefghijkl])/);
  assert.match(html, /fuel-command\.js\?v=(?:023[ef]|024[abcdefghijkl])/);
  assert.match(html, /app\.js\?v=(?:023[ef]|024[abcdefghijkl])/);
  assert.match(worker, /coach-dominion-(?:023[ef]|024[abcdefghijkl])-v1/);
  assert.match(worker, /meal-execution\.js\?v=(?:023[ef]|024[abcdefghijkl])/);
});
