...
test("023B rotates the mutable shell and caches both Fuel engines", () => {
  assert.match(html, /styles\.css\?v=023[bcde]/);
  assert.match(html, /fuel-command\.js\?v=023[bcde]/);
  assert.match(html, /app\.js\?v=023[bcde]/);
  assert.match(worker, /coach-dominion-023[bcde]-v1/);
  assert.match(worker, /fuel-calendar\.js\?v=023b/);
  assert.match(worker, /fuel-command\.js\?v=023[bcde]/);
});
