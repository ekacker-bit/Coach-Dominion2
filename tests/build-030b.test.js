...
test("Build 030B is identifiable, cached, and production gated", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030[BCDEFGHIJK]\.1"/);
  ...
  assert.match(health, /release: "030[BCDEFGHIJK]\.1"/);
  assert.match(workflow, /npm run test:030[bcdefghijk]/);
  assert.match(workflow, /--expected-release 030[BCDEFGHIJK]\.1/);
