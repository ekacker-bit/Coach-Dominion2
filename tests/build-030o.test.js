...
test("030O remains cache-busted and guarded by the current production release", () => {
  assert.match(html, /coach-dominion-release" content="030[OPQ]\.1"/);
  ...
  assert.match(app, /register\("\/sw\.js\?v=030[opq]"/);
  assert.match(health, /release: "030[OPQ]\.1"/);
  assert.match(health, /rankHandoff: "earned-rank-acknowledged"/);
  assert.match(workflow, /npm run test:030[opq]/);
  assert.match(workflow, /--expected-release 030[OPQ]\.1/);
