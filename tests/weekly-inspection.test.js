...
const completedWeek = { asOfDate: dates.at(-1) };

const complete = aggregateWeeklyCompliance(week('completed'), dates[0], completedWeek);
...
], dates[0], completedWeek);
assert.equal(mixed.score, (32 * 100 + 2 * 50) / 35, '2. underlying observations aggregate exactly');

const excused = aggregateWeeklyCompliance(week(['completed', 'excused', 'completed', 'completed', 'completed']), dates[0], completedWeek);
...
const na = aggregateWeeklyCompliance(week(['completed', 'not_applicable', 'completed', 'completed', 'completed']), dates[0], completedWeek);
assert.equal(na.score, 100, '4. N/A observations leave denominator');
assert.equal(na.evidenceCoverage, 100);

const missing = aggregateWeeklyCompliance([record(dates[0], 'completed')], dates[0], completedWeek);
...
const poor = aggregateWeeklyCompliance(week('missed'), dates[0], completedWeek);
...
const unscored = aggregateWeeklyCompliance(week('excused'), dates[0], completedWeek);
...
const ranked = aggregateWeeklyCompliance(week(['completed', 'partial', 'missed', 'completed', 'partial']), dates[0], completedWeek);
...
const aggregate = aggregateWeeklyCompliance(source, dates[0], completedWeek);
...
const invalid = aggregateWeeklyCompliance(week(['invalid', 'completed', 'completed', 'completed', 'completed']), dates[0], completedWeek);
...
aggregateWeeklyCompliance(immutableInput, dates[0], completedWeek);
assert.deepEqual(immutableInput, immutableCopy, '16. aggregation does not mutate daily records');

const precision = aggregateWeeklyCompliance([record(dates[0], ['completed', 'partial', 'partial', 'excused', 'not_applicable'])], dates[0], completedWeek);
...
const mondayInProgress = aggregateWeeklyCompliance(
  [record('2026-07-27', ['completed', 'completed', 'completed', 'completed', ''])],
  '2026-07-27',
  { asOfDate: '2026-07-27', calculatedAt: '2026-07-27T18:00:00.000Z' }
);
assert.equal(mondayInProgress.evidenceCoverage, 80, '17b. future days do not dilute evidence through today');
assert.equal(Math.round(mondayInProgress.projectedFullWeekCoverage), 11, '17c. full-week projection remains separately visible');
assert.equal(mondayInProgress.elapsedDayCount, 1);
assert.equal(mondayInProgress.inspectionStatus, 'PROVISIONAL');
assert.equal(mondayInProgress.scoreIsProvisional, true);
assert.deepEqual(mondayInProgress.missingRequiredDomains, ['nutrition']);
assert.equal(mondayInProgress.dailyEvidence[1].periodState, 'FUTURE');
assert.throws(() => finalizeWeeklyInspectionSnapshot(mondayInProgress), /after the inspection week ends/);

const allTied = aggregateWeeklyCompliance([record('2026-07-27', 'completed')], '2026-07-27', { asOfDate: '2026-07-27' });
assert.equal(allTied.domainRankingTie, true);
assert.deepEqual(allTied.weakestDomains, [], '17d. a full tie does not invent a weakest domain');
