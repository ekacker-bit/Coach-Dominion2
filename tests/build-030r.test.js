...
test("030R is cache-busted and production gated", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030[RSTU]\.1"/);
  ...
  assert.match(app, /register\("\/sw\.js\?v=030[rstu]"/);
  assert.match(health, /release: "030[RSTU]\.1"/);
  assert.match(health, /executionAuthority: "signed-week-reconciled"/);
  assert.match(workflow, /npm run test:030[rstu]/);
  assert.match(workflow, /--expected-release 030[RSTU]\.1/);
