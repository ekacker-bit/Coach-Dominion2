...
check("unified fuel command", html.includes('class="fuel-command-center"') && app.includes("DominionFuelCommand.buildFuelCommand"), "023C unified Fuel surface is missing");
check("fuel command engine", size("assets/js/fuel-command.js") >= 6500 && read("assets/js/fuel-command.js").includes('const VERSION = "023C.1"'), "023C Fuel engine appears incomplete");
...
check("fasting protocol engine", size("assets/js/intermittent-fasting.js") >= 8500 && read("assets/js/intermittent-fasting.js").includes('const VERSION = "023C.1"'), "023C fasting protocol engine appears incomplete");
check("fasting protocol integration", html.includes('id="intermittent-fasting-form"') && app.includes("buildCurrentFastingContext") && app.includes('class="fuel-fasting-brief'), "023C fasting protocol integration is missing");
check("stylesheet version", html.includes('/assets/styles.css?v=023c'), "app.html is not using the 023C stylesheet");
check("application version", html.includes('/assets/js/app.js?v=023c'), "app.html is not using the 023C application");
check("cache version", worker.includes('coach-dominion-023c-v1'), "service-worker cache was not rotated");
check("cached stylesheet", worker.includes('/assets/styles.css?v=023c'), "service worker is caching the wrong stylesheet");
check("cached application", worker.includes('/assets/js/app.js?v=023c'), "service worker is caching the wrong application");
...
check("cached fasting protocol", worker.includes('/assets/js/intermittent-fasting.js?v=023c'), "service worker is not caching the fasting protocol engine");
check("cached fuel command", worker.includes('/assets/js/fuel-command.js?v=023c'), "service worker is not caching the Fuel engine");
