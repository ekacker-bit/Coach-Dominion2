...
check("fuel command engine", size("assets/js/fuel-command.js") >= 6500 && read("assets/js/fuel-command.js").includes('const VERSION = "023D.1"'), "023D Fuel engine appears incomplete");
...
check("stylesheet version", html.includes('/assets/styles.css?v=023d'), "app.html is not using the 023D stylesheet");
check("application version", html.includes('/assets/js/app.js?v=023d'), "app.html is not using the 023D application");
check("cache version", worker.includes('coach-dominion-023d-v1'), "service-worker cache was not rotated");
check("cached stylesheet", worker.includes('/assets/styles.css?v=023d'), "service worker is caching the wrong stylesheet");
check("cached application", worker.includes('/assets/js/app.js?v=023d'), "service worker is caching the wrong application");
...
check("cached fuel command", worker.includes('/assets/js/fuel-command.js?v=023d'), "service worker is not caching the Fuel engine");
