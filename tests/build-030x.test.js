...
test("030X is cache-busted and production gated", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030[XYZ]\.1"/);
  ...
  assert.match(app, /register\("\/sw\.js\?v=030[xyz]"/);
  assert.match(health, /release: "030[XYZ]\.1"/);
  assert.match(health, /continuityRecovery: "one-action-account-restored"/);
  assert.match(workflow, /npm run test:030x/);
  assert.match(workflow, /--expected-release 030[XYZ]\.1/);
});
