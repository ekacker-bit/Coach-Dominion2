...
test("030D release identity, shell, and production gate advance together", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030[DEFGHIJKLMN]\.1"/);
  ...
  assert.match(health, /release: "030[DEFGHIJKLMN]\.1"/);
  assert.match(workflow, /npm run test:030[defghijklmn]/);
  assert.match(workflow, /--expected-release 030[DEFGHIJKLMN]\.1/);
