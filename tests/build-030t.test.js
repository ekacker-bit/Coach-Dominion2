...
test("030T is cache-busted and production gated", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030[TUVW]\.1"/);
  ...
  assert.match(app, /register\("\/sw\.js\?v=030[tuvw]"/);
  assert.match(health, /release: "030[TUVW]\.1"/);
  assert.match(health, /nextDayCommand: "certified-handoff"/);
  assert.match(workflow, /npm run test:030[tuvw]/);
  assert.match(workflow, /--expected-release 030[TUVW]\.1/);
