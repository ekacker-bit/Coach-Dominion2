...
test("030I is cache-busted and production gated", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030[IJKLMN]\.1/);
  ...
  assert.match(app, /register\("\/sw\.js\?v=030[ijklmn]"/);
  assert.match(health, /release: "030[IJKLMN]\.1"/);
  assert.match(health, /closedLoopCoaching: "evidence-to-next-day"/);
  assert.match(workflow, /npm run test:030[ijklmn]/);
  assert.match(workflow, /--expected-release 030[IJKLMN]\.1/);
