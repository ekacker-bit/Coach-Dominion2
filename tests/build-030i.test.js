...
test("030I is cache-busted and production gated", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030[IJKLMNOPQRSTUV]\.1/);
  ...
  assert.match(app, /register\("\/sw\.js\?v=030[ijklmnopqrstuv]"/);
  assert.match(health, /release: "030[IJKLMNOPQRSTUV]\.1"/);
  assert.match(health, /closedLoopCoaching: "evidence-to-next-day"/);
  assert.match(workflow, /npm run test:030[ijklmnopqrstuv]/);
  assert.match(workflow, /--expected-release 030[IJKLMNOPQRSTUV]\.1/);
