...
test("Build 029N makes reconciliation the governing action", () => {
  ...
  assert.match(html, /coach-dominion-release" content="(?:029[NO]|030A)\.1"/);
...
test("Build 029N is responsive, cached, and release-gated", () => {
  ...
  assert.match(workflow, /npm run test:(?:029[no]|030a)/i);
  assert.match(workflow, /--expected-release (?:029[NO]|030A)\.1/);
});
