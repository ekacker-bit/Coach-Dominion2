...
test("022D loads one plan command after the Progress Review engine", () => {
  ...
  const application = html.indexOf('/assets/js/app.js?v=022d');
...
test("022D is word-light, responsive, versioned, and cached", () => {
  ...
  assert.match(html, /styles\.css\?v=022d/);
  assert.match(worker, /coach-dominion-022d-v1/);
  assert.match(worker, /plan-command\.js\?v=022d/);
  assert.match(worker, /app\.js\?v=022d/);
});
