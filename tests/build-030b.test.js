...
test("Build 030B is identifiable, cached, and production gated", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030B\.1"/);
  assert.match(worker, /030b-today-in-15-seconds/);
  assert.match(worker, /today-quick-log\.js\?v=030b/);
  assert.match(health, /release: "030B\.1"/);
  assert.match(workflow, /npm run test:030b/);
  assert.match(workflow, /--expected-release 030B\.1/);
