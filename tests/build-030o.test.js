...
test("030O remains cache-busted and guarded by the current production release", () => {
  assert.match(html, /coach-dominion-release" content="030[OPQRSTUVW]\.1"/);
  ...
  assert.match(app, /register\("\/sw\.js\?v=030[opqrstuvw]"/);
  assert.match(health, /release: "030[OPQRSTUVW]\.1"/);
  assert.match(health, /rankHandoff: "earned-rank-acknowledged"/);
  assert.match(workflow, /npm run test:030[opqrstuvw]/);
  assert.match(workflow, /--expected-release 030[OPQRSTUVW]\.1/);
...
test("030O restores handoff receipts through Account Truth", () => {
  assert.match(account, /const VERSION = "030[OPQRSTUVW]\.1"/);
