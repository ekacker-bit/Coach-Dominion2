...
test("030L assets and release identity are production-gated", () => {
  assert.match(html, /coach-dominion-release" content="030[LMNOPQRST]\.1"/);
  ...
  assert.match(health, /release: "030[LMNOPQRST]\.1"/);
  assert.match(health, /weeklyRollover: "commit-to-monday-certified"/);
  assert.match(workflow, /npm run test:030[lmnopqrst]/);
  assert.match(workflow, /--expected-release 030[LMNOPQRST]\.1/);
...
test("030L persists one receipt through closed-loop and Account Truth", () => {
  assert.match(account, /const VERSION = "030[LMNOPQRST]\.1"/);
