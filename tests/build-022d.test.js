...
test("022D loads one plan command after the Progress Review engine", () => {
  ...
  const application = html.search(/\/assets\/js\/app\.js\?v=(?:022[d-g]|023[abcd])/);
...
test("022D is word-light, responsive, versioned, and cached", () => {
  ...
  assert.match(html, /styles\.css\?v=(?:022[d-g]|023[abcd])/);
  assert.match(worker, /coach-dominion-(?:022[d-g]|023[abcd])-v1/);
  assert.match(worker, /plan-command\.js\?v=022d/);
  assert.match(worker, /app\.js\?v=(?:022[d-g]|023[abcd])/);
});
