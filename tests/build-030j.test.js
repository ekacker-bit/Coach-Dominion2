...
test("030J is cache-busted and production gated", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030[JKLMNOPQ]\.1/);
  ...
  assert.match(app, /register\("\/sw\.js\?v=030[jklmnopq]"/);
  assert.match(health, /release: "030[JKLMNOPQ]\.1"/);
  assert.match(health, /decisionProof: "outcome-verified"/);
  assert.match(workflow, /npm run test:030[jklmnopq]/);
  assert.match(workflow, /--expected-release 030[JKLMNOPQ]\.1/);
});
