...
test("023D is responsive and rotates every mutable production asset", () => {
  ...
  assert.match(html, /styles\.css\?v=(?:023[def]|024[abcdefg])/);
  assert.match(html, /fasting-execution\.js\?v=023d/);
  assert.match(html, /fuel-command\.js\?v=(?:023[def]|024[abcdefg])/);
  assert.match(html, /app\.js\?v=(?:023[def]|024[abcdefg])/);
  assert.match(worker, /coach-dominion-(?:023[def]|024[abcdefg])-v1/);
