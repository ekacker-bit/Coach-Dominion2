...
test("030D release identity, shell, and production gate advance together", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030[DEFGHIJK]\.1"/);
  ...
  assert.match(health, /release: "030[DEFGHIJK]\.1"/);
  assert.match(workflow, /npm run test:030[defghijk]/);
  assert.match(workflow, /--expected-release 030[DEFGHIJK]\.1/);
