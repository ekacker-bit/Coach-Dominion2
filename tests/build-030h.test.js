...
test("030H separates signed Contract authority from an unsigned draft", () => {
  ...
  assert.match(app, /Review R\$\{escapeHtml/);
});
...
test("030H is cache-busted and release gated", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030[HIJKLMNOPQRSTUVW]\.1/);
  assert.match(html, /beta-state-integrity\.js\?v=030[hpqrs]/);
  assert.match(worker, /030h-beta-state-integrity/);
  assert.match(app, /register\("\/sw\.js\?v=030[hijklmnopqrstuvw]"/);
  assert.match(health, /release: "030[HIJKLMNOPQRSTUVW]\.1"/);
  assert.match(workflow, /npm run test:030[hijklmnopqrstuvw]/);
  assert.match(workflow, /--expected-release 030[HIJKLMNOPQRSTUVW]\.1/);
});
