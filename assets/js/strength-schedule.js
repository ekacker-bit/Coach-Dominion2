(function (root, factory) {
  ...
  function normalizePreferredDays(daysPerWeek, preferred = []) {
    const count = Math.max(2, Math.min(4, Number(daysPerWeek || 3)));
  ...
  function runningSessions(plan = {}) {
    return plan?.status === "READY" && Array.isArray(plan.sessions) ? plan.sessions : [];
  }
  ...
  function assignmentConflicts(assignment, context = {}) {
    ...
    if (day.core) conflicts.push({ code: "CORE_COMBINED", severity: "ADVISORY", detail: `${day.coreLabel} is also scheduled; complete strength first and keep core controlled.` });
  ...
  function buildWeeklySchedule(plan = {}, history = [], context = {}, options = {}) {
    ...
    const sessionCount = Math.max(2, Math.min(4, Number(plan.profile?.daysPerWeek || plan.sessions.length)));
    ...
    const completedCount = terminalHistory(history, plan.id).length;
