...
test("030G is cache-busted and production gated", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030[GHIJKLMNOP]\.1/);
  ...
  assert.match(app, /register\("\/sw\.js\?v=030[ghijklmnop]"/);
  assert.match(health, /release: "030[GHIJKLMNOP]\.1"/);
  assert.match(health, /executionLedger: "canonical"/);
  assert.match(workflow, /npm run test:030[ghijklmnop]/);
  assert.match(workflow, /--expected-release 030[GHIJKLMNOP]\.1/);
});
