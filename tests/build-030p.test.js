...
test("030P is cache-busted, offline-cached, and production gated", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030[PQRS]\.1"/);
  ...
  assert.match(app, /register\("\/sw\.js\?v=030[pqrs]"/);
  assert.match(health, /release: "030[PQRS]\.1"/);
  assert.match(workflow, /npm run test:030[pqrs]/);
  assert.match(workflow, /--expected-release 030[PQRS]\.1/);
