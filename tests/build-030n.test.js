...
test("030N is cache-busted and guarded by the production release gate", () => {
  assert.match(html, /coach-dominion-release" content="030[NOPQRSTUV]\.1"/);
  ...
  assert.match(app, /register\("\/sw\.js\?v=030[nopqrstuv]"/);
  assert.match(health, /release: "030[NOPQRSTUV]\.1"/);
  assert.match(health, /rankAdvancement: "finalized-proof-certified"/);
  assert.match(workflow, /npm run test:030[nopqrstuv]/);
  assert.match(workflow, /--expected-release 030[NOPQRSTUV]\.1/);
...
test("030N makes certified history account truth and the authority for rank", () => {
  assert.match(account, /const VERSION = "030[NOPQRSTUV]\.1"/);
...
test("030N replaces browser-only promotion with one compact proof and action", () => {
  assert.match(weekly, /const VERSION = "030[NOPQRSTUV]\.1"/);
