...
test("030M exposes one production-gated release and caches the certification engine", () => {
  assert.match(html, /coach-dominion-release" content="030[MNOPQRSTUVW]\.1"/);
  ...
  assert.match(app, /register\("\/sw\.js\?v=030[mnopqrstuvw]"/);
  assert.match(health, /release: "030[MNOPQRSTUVW]\.1"/);
  assert.match(health, /weekExecution: "assignment-outcomes-certified"/);
  assert.match(workflow, /npm run test:030[mnopqrstuvw]/);
  assert.match(workflow, /--expected-release 030[MNOPQRSTUVW]\.1/);
...
test("030M survives reload through Account Truth without a new persistence silo", () => {
  assert.match(account, /const VERSION = "030[MNOPQRSTUVW]\.1"/);
...
test("030M stays compact across Review, Calendar, Today, and phone layouts", () => {
  ...
  assert.match(weeklyExperience, /const VERSION = "030[MNOPQRSTUVW]\.1"/);
