...
test("023A rotates the mutable shell and caches the Fuel engine", () => {
  assert.match(html, /styles\.css\?v=(?:023[abcdef]|024[ab])/);
  assert.match(html, /fuel-command\.js\?v=(?:023[abcdef]|024[ab])/);
  assert.match(html, /app\.js\?v=(?:023[abcdef]|024[ab])/);
  assert.match(worker, /coach-dominion-(?:023[abcdef]|024[ab])-v1/);
  assert.match(worker, /fuel-command\.js\?v=(?:023[abcdef]|024[ab])/);
  assert.match(worker, /app\.js\?v=(?:023[abcdef]|024[ab])/);
});
