...
test("030D release identity, shell, and production gate advance together", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030D\.1"/);
  ...
  assert.match(worker, /030d-recruit-journey-certification/);
  assert.match(worker, /journey-continuity\.js\?v=030d/);
  assert.match(health, /release: "030D\.1"/);
  assert.match(workflow, /npm run test:030d/);
  assert.match(workflow, /--expected-release 030D\.1/);
