...
test("Build 030B is identifiable, cached, and production gated", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030[BCDEFGH]\.1"/);
  ...
  assert.match(health, /release: "030[BCDEFGH]\.1"/);
  assert.match(workflow, /npm run test:030[bcdefgh]/);
  assert.match(workflow, /--expected-release 030[BCDEFGH]\.1/);
