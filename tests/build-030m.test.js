...
test("030M exposes one production-gated release and caches the certification engine", () => {
  assert.match(html, /coach-dominion-release" content="030M\.1"/);
  ...
  assert.match(app, /register\("\/sw\.js\?v=030m"/);
  assert.match(health, /release: "030M\.1"/);
  assert.match(health, /weekExecution: "assignment-outcomes-certified"/);
  assert.match(workflow, /npm run test:030m/);
  assert.match(workflow, /--expected-release 030M\.1/);
...
test("030M survives reload through Account Truth without a new persistence silo", () => {
  assert.match(account, /const VERSION = "030M\.1"/);
...
test("030M stays compact across Review, Calendar, Today, and phone layouts", () => {
  ...
  assert.match(weeklyExperience, /const VERSION = "030M\.1"/);
  assert.match(weeklyExperience, /id="week-execution-certification"/);
  assert.match(html, /weekly-advancement\.js\?v=026d-030c-030e-030k-030m/);
  assert.match(worker, /weekly-advancement\.js\?v=026d-030c-030e-030k-030m/);
