...
test("030F is cache-busted and production gated", () => {
  ...
  assert.match(html, /coach-dominion-release" content="030F\.1/);
  assert.match(worker, /030f-strength-calendar-execution/);
  assert.match(app, /register\("\/sw\.js\?v=030f"/);
  assert.match(health, /release: "030F\.1"/);
  assert.match(health, /strengthAssignment: "calendar-linked"/);
  assert.match(workflow, /npm run test:030f/);
  assert.match(workflow, /--expected-release 030F\.1/);
});
